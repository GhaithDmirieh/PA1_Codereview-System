...
\begin{description}
	...
	\begin{enumerate}
		...
		\item Review ist nur vor dem Zusammenführen von einer Feature-Branche mit dem Master-Branche Möglich.
		\item Um zu hosten in Bitbucketsever steht für mehr als 5 Personen einen monatlichen Beitrag.\\ Und für Self-hosten (auf eigenem Server) besteht eine einmalige Zahlung.\\
		...
		\begin{enumerate}
			\item One-to-One Reviews: Der Autor startet die Review (Der Autor ist also in diesem Workflow der Moderator), danach kommentiert der Reviewer und der Autor kann auf diese
				...
				\begin{figure}[H]
					...
					\caption[Crucible: one-to-one-review]{Crucible Ablaufsmöglichkeit,\\ Quelle: Atlassian Crucible-Workflow}
					\label{fig:one-to-one-workflow}
				\end{figure}
				\change{Füge eine richtige Quelle hinzu}
			...
			\item Formal group reviews: Der Autor startet die Review und ladet die Reviewer ein. So können die Review diese diskutieren und dementsprechend bekommen die Reviewer 
				...
				\begin{figure}[H]
					...
					\caption[Crucible: Formal group review]{Crucible Ablaufsmöglichkeit,\\ Quelle: Atlassian Crucible-Workflow}
...
\begin{itemize}
	...
	\item \textbf{Reviews Workflow}: Die Review kann in Helix Teamhub eingerichtet werden, so kann das Mitglied, das die Review erstellte, entscheiden, wer die Reviewer
		 sind und welche Änderungen in welchem Branche überprüft werden sollen.	\\ Ein Beispiel für einen möglichen Workflow:
		  ...
		  Es wird zuerst ein Projekt erstellt und zu diesem Projekt wird ein Repository hinzugefügt. Durch git wird eine Branche in diesem Repository
		  erstellt. Der Entwickler arbeitet an dieser Branche. Sobald er eine Review braucht, erstellt er eine. In den Einstellungen sucht man die Branche aus, dann werden Reviewer
	...
	\item \textbf{Workflow}: Gerrit hat nur einen Reviews Ablauf, der nach diesen Schritten erfolgt:
		\begin{enumerate}
			...
			\begin{itemize}
				...
				\item Manuell durch git. \unsure{ERKLÄRUNG}
			\end{itemize}
			Hinweis: Nicht jeder Benutzer kann ein Gerrits Projekt erstellen, denn das braucht die Admins Rechte.		
			
			\item Features entwickeln, dann die commiten und pushen. In diesem Fall pusht man die Änderungen nicht in die Master Branche, sondern in einer Gerrits Branche.
			
				Diese Branche benutzt Gerrit für die Review. Dementsprechend kommt man mit \textit{git diff} (git Befehl, um alte und neue Änderungen zu vergleichen) zum Vergleich
			...
			\begin{figure}[H]
				...
				\caption[Gerrit]{Reviews Webseite,\\ Quelle: kommt bald}
			...
			\item Die Reviewer können entweder die Review manuell finden oder bekommen die eine E-Mail(wenn sie vom Autor als Reviewer gemerkt sind), dass eine Review
				 ...
				 Die Reviewer schauen die Änderungen, antworten auf sie und am Ende bewerten sie. Die Bewertung ist in Gerrit vorgeschrieben.
				 ...
				 \begin{itemize}
				 	\item Sind die Änderungen fehlerhaft oder fehlt etwas, was der Autor beachten musste, so fordern die Reviewer den Autor, die Änderungen zu bearbeitet
				 		und nochmal in die Gerrits Branche zu pushen.
...
\begin{description}
	...
	\begin{enumerate}
		...
		\item Jede Repository kann unendliche Anzahl von Branchen enthalten.
	...
	\begin{enumerate}
		...
		\item Gerrit fordert gute Git-Kenntnisse.
	\end{enumerate}
\end{description}
...
\begin{itemize}
	...
	\begin{itemize}
		\item Die Review kann durch eine Pull-Anfrage gestartet werden. Die Abbildung \ref{fig:Forking-workflow} vom Abschnitt \ref{subsec:Bitbucket} zeigt den Ablauf
			eine Pull-Anfrage. Also der Entwickler arbeitet in einer von Master-Branche abgespalteten-Branche macht Änderungen und sendet eine Pull-Anfrage so bald er fertig ist. 
			So Geschieht die Review dementsprechend vor dem Mergen mit der Master-Branche.
...
\begin{description}
	\item [Vorteile] \hfill
		\begin{enumerate}
			...
			\item Dateisuche in allen Repositorys sowie Volltextsuche \& Indizierung vom Quellcode.
...
Github ist eine webbasiert Plattform für das Management vom Quelltext. Github arbeitet ausschließlich mit dem \ac{VCS} Git. Es bietet die Gelegenheit, unbegrenzte Anzahl von public/private Repositories auf Githubs Server zu hosten. Es gehört seit Juni. 2018 Microsoft.

\begin{itemize}
	\item \textbf{Art der Review}: \textit{pre- and post-commit}
	\item \textbf{Entwickler}: Tom Preston-Werner, Chris Wanstrath, P. J. Hyett.
	\item \textbf{Workflow}: Die Review funktioniert in Github durch eine Pull-Anfrage, wie beim Bitbuckets Workflow in \ref{subsec:Bitbucket} arbeitet der Entwickler an einer
		speziellen Branche, macht Änderungen und startet eine Pull-Anfrage und wählt die Reviewer, die eine Nachricht bekommen, dass sie zur Überprüfung gefragt sind. Nach Annahme der
		Änderungen, können diese automatisch mit der Master-Branche zusammengeführt werden.
\end{itemize}
